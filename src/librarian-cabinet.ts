...
function saveUserBooks(phone: string, type: 'reservations' | 'favorites' | 'loans', data: number[]) {
...
async function renderUserList() {
    ...
    document.querySelectorAll('.user-select-btn').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            e.stopPropagation();
            const phone = (btn as HTMLElement).getAttribute('data-phone');
...
async function renderUserDetails(phone: string) {
