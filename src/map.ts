class LibraryMap {
    ...
    private async loadSVG(): Promise<void> {
        return new Promise((resolve) => {
            this.svgObject = document.getElementById('library-map') as HTMLObjectElement;
    ...
    private addShelfNumbers(): void {
        ...
        for (let i = 1; i <= 30; i++) {
            ...
            if (shelfElement) {
                const svgElement = shelfElement as unknown as SVGGraphicsElement;
    ...
    private makeShelvesClickable(): void {
        if (!this.svgDocument) return;
